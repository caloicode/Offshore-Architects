...
const ArrowDown = ({ open }) => (
  <svg
    className={`w-4 h-4 ml-1 inline transition-transform duration-300 ${open ? "rotate-180" : ""}`}
    fill="none"
    stroke="currentColor"
    viewBox="0 0 24 24"
  >
    ...
    <nav className="bg-primary shadow-md fixed w-full z-50 transition-colors duration-500">
      <div className="px-4 py-3 flex justify-between items-center mx-auto md:mx-0 w-full">
        <a href="/" className="flex items-center">
          <img src="https://placehold.co/40x40" alt="Logo" className="h-10 w-10 rounded-full mr-2" />
          <span className="text-primary text-xl font-bold">Offshore Architects PH</span>
        </a>
        ...
        <div className="hidden lg:flex items-center space-x-6">
          <a href="/" className="text-primary hover:text-gray-600">Home</a>
          <div className="relative">
            ...
            >
              Services <ArrowDown open={isServicesOpen} />
            </button>
            {isServicesOpen && (
              <div
                  ...
                  <div key={index}>
                    <button
                      ...
                      className={`w-full text-left text-primary hover:text-gray-600 focus:outline-none px-3 py-2 rounded ${
                        openCategory === service.category ? "bg-gray-100 dark:bg-gray-800" : ""
                      }`}
                      aria-expanded={openCategory === service.category}
                    >
                      {service.category} <ArrowDown open={openCategory === service.category} />
                    </button>
          ...
          <a href="#portfolio" className="text-primary hover:text-gray-600">Portfolio</a>
          <a href="#about" className="text-primary hover:text-gray-600">About Us</a>
          <a href="#footer" className="text-primary hover:text-gray-600">Contact</a>
    ...
    <div className="p-4 h-full flex flex-col">
      ...
      <div className="flex-grow overflow-y-auto">
        ...
        <div>
          <button
            onClick={toggleServices}
            className={`w-full text-left py-3 text-primary rounded px-3 flex justify-between items-center ${
              isServicesOpen
            ...
            <div className="pl-2">
              {servicesData.map((service, index) => (
                <div key={index} id={`category-${service.category}`}>
                  <button
                    onClick={() => toggleCategory(service.category)}
                    className={`w-full text-left py-3 text-primary rounded px-4 flex justify-between items-center ${
                      openCategory === service.category
                        ? "bg-gray-100/50 dark:bg-gray-800/50"
                        : "hover:bg-gray-100/30 dark:hover:bg-gray-800/30"
                    }`}
